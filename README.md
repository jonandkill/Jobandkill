...
- NCS 공통정보 사전 수집·페이지 체크포인트·전체 수집 검증 및 훈련기준 CSV 가져오기
- 공고와 분리된 NCS 카탈로그 검색·선택 적용, 제공되지 않은 KSA/수행준거 표시
- 정보원별 구현·이용권한·문서 추출 공백 진단 (`coverage`, `/api/data-coverage`)
...
직무·직업 데이터가 우선입니다. 상세 정보원·권한·실제 수집 범위는
[정부 직무정보 수집 설계](docs/government-data.md)를 기준으로 합니다.
NCS 능력단위 목록을 다 받았다고 모든 지식·기술·태도나 기관 직무기술서를
확보한 것은 아닙니다. 서버 가격·리전 선택은 실제 수집량과 부하 검증 후 결정합니다.

| 범위 | 공식 소스 | 구현 상태 |
|---|---|---|
| NCS 능력단위 기준 DB | 공공데이터포털 NCS 훈련기준 CSV | 실제 파일 13,442행 적재 검증, KSA 본문 미포함 |
| NCS 공통정보 갱신 | 한국산업인력공단 공통정보 API | 수집·재개·전체검증 구현, 승인키 실응답 미검증 |
